...
// Progress counts every milestone AND every syllabus point nested inside
// every milestone — completing either moves the needle.
    ...
    milestones: [
      {
        ...
        projectRef: null,
        syllabus: [
          { id: 'r1', title: 'Web API fundamentals', details: 'Routing, controllers vs minimal APIs, model binding.', done: true, projectRef: null },
          { id: 'r2', title: 'EF Core + SQL Server', details: 'DbContext, migrations, LINQ queries against SQL Server.', done: true, projectRef: null },
          { id: 'r3', title: 'Auth (Identity/JWT)', details: 'Token issuing, refresh tokens, role-based auth.', done: true, projectRef: { type: 'project', id: 'p1', title: 'Personal OS' } },
          { id: 'r4', title: 'Middleware pipeline', details: 'Order matters — auth before authorization, logging placement.', done: false, projectRef: null },
        ],
      },
      {
        id: 'ms2',
        ...
        projectRef: { type: 'project', id: 'p1', title: 'Personal OS' },
        syllabus: [],
      },
    ],
    resources: [
    ...
    milestones: [
      {
        ...
        projectRef: null,
        syllabus: [
          { id: 'r5', title: 'Hooks and component patterns', details: 'useState, useEffect, custom hooks, composition over inheritance.', done: true, projectRef: null },
          { id: 'r6', title: 'TanStack Query', details: 'Query keys, cache invalidation vs. direct cache writes.', done: true, projectRef: null },
          { id: 'r7', title: 'Testing (RTL)', details: 'React Testing Library — testing behavior, not implementation.', done: false, projectRef: null },
        ],
      },
    ],
    ...
    milestones: [
      {
        ...
        projectRef: null,
        syllabus: [{ id: 'r8', title: 'Dart basics', details: 'Syntax, null safety, async/await.', done: false, projectRef: null }],
      },
    ],
    ...
    milestones: [
      {
        ...
        projectRef: null,
        syllabus: [
          { id: 'r9', title: 'Execution plans', details: 'Reading actual vs. estimated plans, spotting table scans.', done: true, projectRef: null },
          { id: 'r10', title: 'Indexing strategy', details: 'Covering indexes vs. composite indexes.', done: true, projectRef: null },
        ],
      },
      {
        id: 'ms2b',
        ...
        projectRef: { type: 'project', id: 'p2', title: 'ERP System' },
        syllabus: [],
      },
    ],
...
export const mockSkillService: SkillService = {
  ...
  async toggleMilestone(skillId: string, milestoneId: string): Promise<Skill> {
    ...
    return updateSkill(skillId, (s) =>
      recompute(mapMilestone(s, milestoneId, (m) => ({ ...m, done: !m.done }))),
    );
  },
  ...
  async addMilestone(skillId: string, title: string): Promise<Skill> {
    ...
    return updateSkill(skillId, (s) =>
      recompute({
        ...s,
        milestones: [...s.milestones, { id: `ms${Date.now()}`, title, done: false, projectRef: null, syllabus: [] }],
      }),
    );
  },
  ...
  async setMilestoneProject(skillId: string, milestoneId: string, projectRef: LinkRef | null): Promise<Skill> {
    await delay(200);
    return updateSkill(skillId, (s) => ({
      ...mapMilestone(s, milestoneId, (m) => ({ ...m, projectRef })),
      updatedAt: today(),
    }));
  },
  ...
  async addSyllabusItem(skillId: string, milestoneId: string, title: string): Promise<Skill> {
    ...
    return updateSkill(skillId, (s) =>
      recompute(
        mapMilestone(s, milestoneId, (m) => ({
          ...m,
          syllabus: [...m.syllabus, { id: `sy${Date.now()}`, title, details: '', done: false, projectRef: null }],
        })),
      ),
    );
  },
  ...
  async setSyllabusItemProject(skillId: string, milestoneId: string, itemId: string, projectRef: LinkRef | null): Promise<Skill> {
    await delay(200);
    return updateSkill(skillId, (s) => ({
      ...mapMilestone(s, milestoneId, (m) => ({
        ...m,
        syllabus: m.syllabus.map((i) => (i.id === itemId ? { ...i, projectRef } : i)),
      })),
      updatedAt: today(),
    }));
  },
